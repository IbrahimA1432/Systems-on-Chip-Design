...
extern volatile uint32_t *h2p_lw_hex_addr;
...
static unsigned char msg[] = {
    ...
    57,   // C
    0     // blank
}; // Displays "CHILL" (depending on digit order)
...
void SEG7_Hex(unsigned long Data, unsigned char point_mask){
    ...
    for(int i=0;i<SEG7_NUM;i++){
        unsigned char seg_mask = szMap[Data & 0x0F];
        Data >>= 4;
        if (point_mask & mask) seg_mask |= 0x80;
        mask <<= 1;
        SEG7_SET(i, seg_mask);
    }
}
...
void SEG7_Decimal(unsigned long Data, unsigned char point_mask){
    unsigned char mask = 0x01;
    for(int i=0;i<SEG7_NUM;i++){
        unsigned char seg_mask = szMap[Data % 10];
        Data /= 10;
        if (point_mask & mask) seg_mask |= 0x80;
        mask <<= 1;
        SEG7_SET(i, seg_mask);
